...
def test_version_markers_are_updated_to_v092():
    assert 'version = "0.9.8"' in (ROOT / "pyproject.toml").read_text(encoding="utf-8")
    assert '__version__ = "0.9.8"' in (ROOT / "src/cs2pov/__init__.py").read_text(encoding="utf-8")
    assert "CS2 POV Translator v0.9.8" in (ROOT / "README.zh.md").read_text(encoding="utf-8")
    assert "CS2 POV Translator v0.9.8" in (ROOT / "Start_CS2_POV_Translator.bat").read_text(encoding="utf-8")
