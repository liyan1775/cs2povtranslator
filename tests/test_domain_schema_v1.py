...
def test_domain_schema_error_exposes_stable_diagnostic_fields():
    error = DomainSchemaError(
        "time_range_invalid", "时间范围无效。", "请修正后重试。", "cue.start_us"
    )
    assert (
        str(error) == "时间范围无效。"
        and error.code == "time_range_invalid"
        and error.message == "时间范围无效。"
        and error.action == "请修正后重试。"
        and error.path == "cue.start_us"
    )
...
def test_current_schema_accepts_only_exact_integer_one():
    assert (
        ...
        and require_current_schema({"schema_version": 1}, "document") == 1
    )
...
def test_probability_is_finite_and_between_zero_and_one():
    assert (
        require_probability(0, "confidence") == 0.0
        and require_probability(0.86, "confidence") == 0.86
        and require_probability(1, "confidence") == 1.0
    )
...
def test_demo_time_has_a_bounded_current_version_range():
    assert (
        require_int(
            MAX_DEMO_TIME_US, "demo_time_us", minimum=0, maximum=MAX_DEMO_TIME_US
        )
        == MAX_DEMO_TIME_US
    )
    with pytest.raises(DomainSchemaError) as e:
        require_int(
            MAX_DEMO_TIME_US + 1, "demo_time_us", minimum=0, maximum=MAX_DEMO_TIME_US
        )
    assert e.value.code == "domain_field_invalid"
...
def test_canonical_json_fingerprint_is_derived_and_order_independent():
    ...
    assert (
        ...
        and content_fingerprint(left) == content_fingerprint(right)
        and len(content_fingerprint(left)) == 64
    )
    with pytest.raises(DomainSchemaError) as e:
