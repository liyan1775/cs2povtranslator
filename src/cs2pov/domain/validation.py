...
def _index(values: object, path: str, attr: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise DomainSchemaError("domain_field_invalid", "领域引用无效。", "请修正后重试。", path) from exc
...
def validate_voice_activity_against_timeline(activity: VoiceActivityCue, timeline: DemoTimeline) -> None:
    ...
    for anchor_id in activity.anchor_ids:
        ...
        if anchor.source_clock is not SourceClock.COMPACT_AUDIO_SAMPLE or anchor.source_stream_id != activity.player_id:
            _error("time_anchor_invalid", "anchor_ids")
        if not (anchor.demo_range.start_us <= activity.time_range.start_us and activity.time_range.end_us <= anchor.demo_range.end_us):
            _error("cue_reference_invalid", "time_range")
    if timeline.round_for_time(activity.time_range.start_us) is None or timeline.round_for_time(activity.time_range.end_us - 1) is None:
        _error("cue_reference_invalid", "time_range")
...
def validate_transcript_against_timeline(
    ...
    if not isinstance(transcript, TranscriptCue) or not isinstance(timeline, DemoTimeline):
        _error("domain_field_invalid", "transcript")
    players = {player.player_id for player in timeline.descriptor.players}
    if transcript.player_id not in players:
        _error("player_reference_invalid", "player_id")
    if transcript.round_id is None or transcript.round_id not in _round_index(timeline):
        _error("cue_reference_invalid", "round_id")
    round_value = _round_index(timeline)[transcript.round_id]
    if not (round_value.time_range.start_us <= transcript.time_range.start_us and transcript.time_range.end_us <= round_value.time_range.end_us):
        _error("cue_reference_invalid", "time_range")
    anchors = _items(timeline.anchors, "anchors")
    if transcript.source_clock is SourceClock.COMPACT_AUDIO_SAMPLE and transcript.source_stream_id != transcript.player_id:
        _error("cue_reference_invalid", "source_stream_id")
    try:
        mapped = map_source_range(anchors, transcript.source_clock, transcript.source_stream_id, transcript.source_start, transcript.source_end)
    except DomainSchemaError as exc:
        raise exc
    if mapped.anchor_ids != transcript.anchor_ids or mapped.envelope != transcript.time_range:
    ...
    for activity_id in transcript.voice_activity_ids:
        ...
        if activity.player_id != transcript.player_id or not (activity.time_range.start_us <= transcript.time_range.start_us and transcript.time_range.end_us <= activity.time_range.end_us):
...
def validate_understanding_document_graph(
    ...
    assigned = [cue for cue in transcript_values if isinstance(cue, TranscriptCue) and cue.round_id == document.round_id]
    if any(not isinstance(cue, TranscriptCue) for cue in transcript_values):
        _error("domain_field_invalid", "transcripts")
    assigned.sort(key=lambda cue: (cue.time_range.start_us, cue.time_range.end_us, cue.cue_id))
    request = {"round_id": document.round_id, "transcript_cues": [cue.to_dict() for cue in assigned]}
    ...
    if configuration is None or configuration.capability is not ModelCapability.UNDERSTANDING_TRANSLATION:
    ...
    if len(by_cue) != len(assigned) or {result.cue_id for result in results} != set(by_cue) or len({result.cue_id for result in results}) != len(results):
    ...
    if invocation is None or invocation.configuration_snapshot_id != document.model_configuration_snapshot_id or invocation.task_id != document.round_id:
        _error("invocation_reference_invalid", "invocation_record_id")
    ordered_results = sorted(results, key=lambda result: (by_cue[result.cue_id].time_range.start_us, by_cue[result.cue_id].time_range.end_us, result.cue_id))
    ...
    response = {"round_id": document.round_id, "results": [result.to_dict() for result in ordered_results]}
    if invocation.request_content_fingerprint != content_fingerprint(request) or invocation.response_content_fingerprint != content_fingerprint(response):
...
def compose_draft_timeline(
    ...
    if any(not isinstance(document, RoundUnderstandingDocument) for document in document_values):
        _error("domain_field_invalid", "documents")
    expected_round_ids = [round_value.round_id for round_value in timeline.rounds.rounds]
    ...
    for document in document_values:
        validate_understanding_document_graph(document, transcript_values, configurations, invocations)
    ...
    for document in ordered_documents:
        for result in document.results:
            ...
            cues.append(DraftCommsCue.from_transcript_and_understanding(transcript, result))
    cues.sort(key=lambda cue: (cue.start_us, cue.end_us, cue.cue_id))
    input_fingerprint = content_fingerprint({"round_understanding": [document.to_dict() for document in ordered_documents]})
    return DraftCommsTimeline(timeline.descriptor.demo_asset_id, "demo-microseconds", input_fingerprint, tuple(cues))
...
def validate_draft_timeline_graph(
    ...
    expected = compose_draft_timeline(timeline, transcripts, documents, configurations, invocations)
    if expected != draft:
...
def validate_reviewed_timeline_graph(
    ...
    if not isinstance(reviewed, ReviewedCommsTimeline) or not isinstance(draft, DraftCommsTimeline) or not isinstance(timeline, DemoTimeline):
    ...
    if reviewed.demo_asset_id != draft.demo_asset_id or reviewed.timebase != "demo-microseconds":
    ...
    for cue in reviewed.cues:
        ...
        if source is None or cue.round_id != source.round_id or cue.player_id != source.player_id or cue.asr_original != source.asr_original or cue.interpreted_source != source.interpreted_source or cue.model_translated_zh != source.translated_zh or cue.model_confidence != source.confidence or cue.evidence != source.evidence:
            _error("cue_reference_invalid", "cues")
        round_value = _round_index(timeline).get(cue.round_id)
        if round_value is None or not (round_value.time_range.start_us <= cue.start_us and cue.end_us <= round_value.time_range.end_us):
            _error("cue_reference_invalid", "cues")
