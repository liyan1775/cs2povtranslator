...
> **2026-08-31 范围纠正：** “历史 Job”只指新版同版本程序生成后，在后续进程中重新列出、打开和继续的 Job。当前不新增旧版 v0.x Job 迁移器，也不实现跨软件版本 Schema 迁移链。阶段 2 的当前设计以 [新版 Job 领域模型、统一时间轴与持久化设计](../superpowers/specs/2026-08-31-new-job-domain-and-timeline-design.md) 为准。

## 1. 执行约束

1. 唯一源码基线已经确定为 `canonical` Git 仓库，远程为 `liyan1775/cs2povtranslator`，目标分支为 `master`。外层与嵌套历史副本仅作验收素材来源；后续实施不得重新初始化或覆盖这些副本。
...
执行状态和跨阶段交付顺序以 [交付里程碑与验收计划](2026-09-05-delivery-milestones.zh.md) 为准；下表保留模块划分，不要求完整模块全部结束后才开始最小产品流程集成。
...
### 任务 1.3：已取消——旧版 Job 迁移器

2026-08-31 经用户确认，不为旧版 v0.x Job 新增目录扫描、迁移报告或幂等导入器。已有只读/恢复兼容能力保留但不扩展，直接进入阶段 2 的新版核心。
...
每个序列化格式包含 `schema_version`。当前阶段只实现和测试当前版本读写；版本不匹配时安全拒绝，不实现迁移链或向后读取。
...
2026-09-03 将本任务拆成两个可独立验收、顺序实施的详细批次：

1. [02C-A：回合任务状态核心](../superpowers/plans/2026-09-03-round-task-state-core.md)：先固定纯领域状态、重试、进度和最小失效规则；
2. [02C-B：回合并行编排运行时](../superpowers/plans/2026-09-03-round-orchestration-runtime.md)：在 02C-A 合并后增加任务持久化、并行、取消、恢复和真实进程回放。

这样 Luna 可以逐批实现，强模型在两个 GitHub 合并门禁分别复核，避免把领域规则、文件一致性和异步并发一次性混在同一审查中。
...
本试验在既有语音链路满足首版覆盖与时间精度要求时不阻塞理解翻译最小流程。先以三个代表性 Demo 完成一次对照并记录采用、保留回退或暂缓结论；只有明确的数据质量阻断项才进入追加修复。不得无界扩展候选工具范围。
...
通用任务状态、持久化、确定性聚合、并发上限、重试、取消和进程恢复由 02C-A/02C-B 交付。本任务复用其实现，仅增加服务商级限流、真实响应错误映射、并发策略参数校准与真实 API 验证，不另建调度器或任务仓储。
...
评测样本准备前置到 02C 实施期间；理解翻译适配器接入前完成基线标注、开发集/保留集划分和验收标准冻结。仅基于合成样例或结构化契约通过，不得宣称真实翻译质量已验收。执行要求见交付里程碑计划。
...
2026-09-05 更新：以下批次保留为完整功能范围索引。实际交付采用里程碑计划中的 M0–M3，优先贯通真实 Demo、理解翻译、人工复核、字幕导出和同版本重开；完整知识库、管理功能与视频能力按后续增量交付。
