...
> **2026-08-31 范围纠正：** 当前不新增旧版 v0.x Job 迁移器，也不实现跨软件版本 Schema 迁移链。“历史 Job”只指新版同版本程序生成后，在后续进程中重新列出、打开和继续的 Job。阶段 2 细化设计以 [新版 Job 领域模型、统一时间轴与持久化设计](../superpowers/specs/2026-08-31-new-job-domain-and-timeline-design.md) 为准。
...
2. Schema/契约测试：所有端口、manifest 和当前版本读写；版本不匹配安全拒绝；
