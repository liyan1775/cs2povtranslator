...
> **2026-08-31 后续范围纠正：** 本文关于“阶段 1.3 旧 Job 扫描/迁移”的后续安排已取消。已有旧 Job 兼容行为保留但不扩展；下一阶段直接建设新版 Job 领域模型、统一时间轴及同版本历史打开能力。详见 [新版 Job 领域模型、统一时间轴与持久化设计](../superpowers/specs/2026-08-31-new-job-domain-and-timeline-design.md)。
...
- 旧 Job 扫描、导入和批量迁移；该能力经 2026-08-31 决策取消，不再列入后续阶段；
...
完成后的下一步改为阶段 2：新版 Job 领域模型、统一时间轴与同版本历史打开。旧版资产迁移闭环不再是当前项目目标。
