...
02E-1 提供当前版本 Job 的本地只读管理界面和 JSON 查询接口。它复用当前版本工作区、DemoAsset 和 Job 仓储，因此页面显示的阶段、运行状态、回合进度和诊断与文件系统中的 Job 保持一致。
...
本版本用于验证框架可测试性和当前 Job 查询链路，暂不提供复核写入、音频播放、任务启动/取消、模型 API 配置或字幕导出按钮。后续子阶段会继续复用同一查询层和当前应用服务，并为写操作增加 writer claim、manifest CAS 和领域校验。
