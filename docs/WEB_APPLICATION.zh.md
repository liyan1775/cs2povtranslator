...
02E-1 提供当前版本 Job 的本地只读管理界面和 JSON 查询接口。02E-2 增加按 Cue 组织的复核查询投影和只读复核页面。它们复用当前版本工作区、DemoAsset 和 Job 仓储，因此页面显示的阶段、运行状态、回合进度和诊断与文件系统中的 Job 保持一致。
...
复核页面地址为 `/jobs/<job_id>/rounds/<round_id>/review`。页面使用稳定的 `data-testid` 和 ARIA 文本状态，逐条展示时间、原始 ASR、解释、翻译、依据和复核状态；页面当前不写入 Job。

## 当前边界

本版本用于验证框架可测试性和当前 Job 查询链路，暂不提供复核写入、音频播放、任务启动/取消、模型 API 配置或字幕导出按钮。当前版本 Job 的语音/ASR 应用服务只把音频作为受管临时缓存使用，未形成可供 Web 安全引用的持久媒体清单，因此复核接口会明确返回音频引用尚未就绪。后续子阶段会继续复用同一查询层和当前应用服务，并为写操作增加 writer claim、manifest CAS 和领域校验。
